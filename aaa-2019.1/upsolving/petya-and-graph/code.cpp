...
int main(){
	...
	cin.tie(0);
	int source = 0, sink = 3000;
	int n,m,x,y,z;
	cin >> n >> m;
	PushRelabel::n = n+m+10;
	for (int i = 1; i <= n; ++i) {
		cin >> x;
		PushRelabel::addEdge(i, sink, x);
	}
	...
	for (int i = 1; i <= m; ++i) {
		cin >> x >> y >> z;
		cnt += z;
		PushRelabel::addEdge(source, i+n, z);
		PushRelabel::addEdge(i+n, x, inf);
		PushRelabel::addEdge(i+n, y, inf);
	}

	cout << cnt - PushRelabel::flow(source, sink) << '\n';

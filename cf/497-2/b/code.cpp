...
int main(){
  ...
  t[0] = max(a[0].first, a[0].second);
  bool ok = true;
  loop1(1, n-1){
    if(min(a[i].first, a[i].second) > t[i-1]){
      ok = false;
      break;
    }
    t[i] = min(a[i].first, a[i].second);
    if(max(a[i].first, a[i].second) <= t[i-1])
      t[i] = max(a[i].first, a[i].second);
  }
